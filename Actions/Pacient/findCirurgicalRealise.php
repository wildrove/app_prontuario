<?php
	...
	$evolutionDate = $findDate->findCirurgicalRealise($regProntuary, $start, $itemsPerPage);

	if (empty($evolutionDate)) {
		header('Location: ../../AlertsHTML/alertNoneEvolutionFound.html');
	}

	$totalRows = $findDate->TotalCirurgicalRealise($regProntuary);
...
<body>
    <div class="container">
        ...
        ?>
        <h1 class="text-center mb-3"  style="margin-top: 120px;">Resumo de Cirurgia</h1>    
        <div>
			 ...
			 <table class="table shadow-lg table-hover table-striped table-bordered">
			     <thead class="thead-dark">
			         <tr class="text-center" style="font-size: 15px">
			            <th scope="col" class="border-right">DATA CRIAÇÃO</th>
			            <th scope="col" class="border-right">PROFISSIONAL</th>
			            <th scope="col" class="border-right">CIRURGIA</th>
			     ...
			     <tbody>
			         ...
			         <tr class="text-center border font-italic">
			              <th scope="row" class="border-right "><?php echo date('d/m/Y', strtotime($rowPacient['DATA_INC'])); ?></th>
			              <td class="border-right"><?php echo $rowPacient['NOME_COMPLETO']; ?></td>
			              <td class="border-right"><?php echo $rowPacient['NOME']; ?></td>
			              <td>
			               <a href="cirurgicalRealiseResume.php?regProntuary=<?php echo $regProntuary . '&regPacient=' . $rowPacient['REGISTRO_PACIENTE'] . '&birthday=' . $birthday . '&mother=' . $mother . '&pacientName=' . $name . '&cirurgicalDate=' . $rowPacient['DATA_INC'] . '&cirurgicalCode=' . $rowPacient['CODIGO_CIRURGIA'] . '&resumeType=' . $resumeType . '&doctor=' . $rowPacient['NOME_COMPLETO'] ?>" class="btn btn-primary">Visualizar</a>
			              </td>
			         </tr>
			            <?php
			        }?>
			...
			<nav align="center" aria-label="Page navigation" style="margin-bottom: 20px;">
					<ul class="pagination mt-3">
					   <li class="page-item <?php if($previousPage == 0){ echo 'disabled';} ?>">
					       <?php 
					           if($previousPage != 0) { ?>
					               <a href="findCirurgicalRealise.php?page=<?php echo $previousPage; ?>&regProntuary=<?php if(isset($_GET['regProntuary']))($_SESSION['regValue'] = $regProntuary); echo $_GET['regProntuary'];?>" style="text-decoration: none;">
					   ...
					   <?php
					        ...
					        if($currentPage > 1){
					        echo "<li class='page-item'><a class='page-link ' href='findCirurgicalRealise.php?page=".$previousPage."&regProntuary=".$_GET['regProntuary']."'>".$previousPage."</a></li>";
					        }
					        echo "<li class='page-item active'><a class='page-link ' href=''>".$currentPage."</a></li>";
					        if($nextPage <= $totalPages){
					          echo "<li class='page-item'><a class='page-link' href='findCirurgicalRealise.php?page=".$nextPage ."&regProntuary=".$_GET['regProntuary']."'>".$nextPage."</a></li>"; 
					        }
					        if($nextPage < $totalPages){
					            echo "<li class='page-item'><span class='page-link' aria-hidden='true'>...</li>";
					        }
					      ...
					      <li class="page-item <?php if($nextPage > $totalPages){echo 'disabled';} ?>">
					           <?php 
					               if($nextPage <= $totalPages) { ?>
					                   <a href="findCirurgicalRealise.php?page=<?php echo $nextPage; ?>&regProntuary=<?php if(isset($_GET['regProntuary']))($_SESSION['regValue'] = $regProntuary); echo $_GET['regProntuary'];?>" style="text-decoration: none;">
